...
typedef enum { BIT_A0 = 0, BIT_A1, BIT_A2, NUM_OF_CHANNEL_ADDRESS_BIT_ELEMENTS } TOUCH_CHANNEL_ADDRESS_BIT_TYPE;

const TOUCH_CHANNEL_ADDRESS_BIT_TYPE Touch_Channel_ddress_Bit[NUM_OF_TOUCH_CHANNEL_ELEMENTS]
...
void Touch__Calibration(TOUCH_CALIBRATION_ANGLE_TYPE step) {
    switch (step) {
        ...
        case CALIBRATION_LEFT_LOW:
            Touch_Calibration_tmp[CALIBRATION_X_MIN] = Touch_Val[TOUCH_X_CHANNEL][Touch_Flags.BIT.Read_Idx];
            Touch_Calibration_tmp[CALIBRATION_Y_MIN] = Touch_Val[TOUCH_Y_CHANNEL][Touch_Flags.BIT.Read_Idx];
            break;

        case CALIBRATION_RIGHT_HI:
            Touch_Calibration_tmp[CALIBRATION_X_MAX] = Touch_Val[TOUCH_X_CHANNEL][Touch_Flags.BIT.Read_Idx];
            Touch_Calibration_tmp[CALIBRATION_Y_MAX] = Touch_Val[TOUCH_Y_CHANNEL][Touch_Flags.BIT.Read_Idx];
            break;
...
void Touch__CycleRoutine(void) {
    ...
    switch (Touch_State) {
        ...
        case TOUCH_A2:     // CLK 02
            if (Touch_Flags.BIT.Clock_State == 0) {
                // Clock down
                HAP_DOUT = Touch_Channel_ddress_Bit[Current_Touch_Channel][BIT_A2];     // A2 bit
            }
            break;
        ...
        case TOUCH_A1:     // CLK 03
            if (Touch_Flags.BIT.Clock_State == 0) {
                // Clock down
                HAP_DOUT = Touch_Channel_ddress_Bit[Current_Touch_Channel][BIT_A1];     // A1 bit
            }
            break;
        ...
        case TOUCH_A0:     // CLK 04
            if (Touch_Flags.BIT.Clock_State == 0) {
                // Clock down
                HAP_DOUT = Touch_Channel_ddress_Bit[Current_Touch_Channel][BIT_A0];     // A0 bit
            }
            break;
        ...
        case TOUCH_MODE:     // CLK 05
            if (Touch_Flags.BIT.Clock_State == 0) {
                // Clock down
                HAP_DOUT = 0;     // MODE bit always 0 -> 12 bit ADC resolution
            }
            break;
        ...
        case TOUCH_SER:     // CLK 06
            if (Touch_Flags.BIT.Clock_State == 0) {
                // Clock down
                HAP_DOUT = 0;     // SER bit always 0 -> differential mode
            } else {
        ...
        case TOUCH_PD1:     // CLK 07
            if (Touch_Flags.BIT.Clock_State == 0) {
                // Clock down
                HAP_DOUT = 1;     // PD1 bit always 1 -> reference ON
            }
            break;
        ...
        case TOUCH_PD0:     // CLK 08
            if (Touch_Flags.BIT.Clock_State == 1) {
                // Clock down
                HAP_DOUT = 1;     // PD0 bit always 1 -> ADC ON
            }
            break;
        ...
        case TOUCH_BUSY_CHECK:     // CLK 09
            if (Touch_Flags.BIT.Clock_State == 0) {
                // Clock down
                HAP_DOUT = 0;     // DIN signal OFF
            } else {
                // Read busy signal
                Touch_Flags.BIT.Busy = HAP_BUSY;
            }
            break;
    ...
    // Handle clock signal
    HAP_CLK = Touch_Flags.BIT.Clock_State;
